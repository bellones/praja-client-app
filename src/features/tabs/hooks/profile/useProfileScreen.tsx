...
const useProfileScreen = () => {
  ...
  const navigation =
    useNavigation<MainTabsScreenProps<'Profile'>['navigation']>();
  ...
  const handleMenuPress = useCallback(
    (menuId: string) => {
      ...
      const screenMap: Record<
        string,
        keyof import('../../../../navigation/types').ProfileStackParamList
      > = {
      ...
      if (screenName) {
        const rootNavigation =
          navigation.getParent<RootStackNavigationProp<'ProfileStack'>>();
        if (rootNavigation) {
